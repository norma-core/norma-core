...
import { airgradient_open_air_o_1pst, arduino_nicla_sense_env, dmesg, hikmicro, ina226, usbvideo, st3215, motors_mirroring, sysinfo, yahboom_dogzilla_lite, normvla, vesc_trampa, victron_smartsolar_mppt, arduino_nicla_sense_me, dfrobot_light_rs485 } from '@/api/proto.js';
...
import DfrobotLightRs485Expanded from '@/components/history/DfrobotLightRs485Expanded';
...
function tryDecodeProtobuf(rawData: Uint8Array): { decoded: unknown; typeName: string } | null {
  const decoders = [
    ...
    { name: 'dfrobot_light_rs485.RxEnvelope', decode: () => dfrobot_light_rs485.RxEnvelope.decode(rawData) },
...
function getAvailableTabs(
  ...
  const isDfrobotLightRs485 = type === 'dfrobot-light-rs485' && data instanceof dfrobot_light_rs485.RxEnvelope;
  ...
  if (isUsbVideo || isUsbVideoTx || isHikmicroThermal || isSt3215 || isSt3215Tx || isMirroring || isVescTrampaTx || isSysinfo || isArduinoNiclaSenseEnv || isArduinoNiclaSenseMe || isIna226 || isDfrobotLightRs485 || isAirGradient || isVictronSmartSolar || isYahboomDogzillaLite || isNormvla) {
...
export default function ExpandedView({ data, type, rawData, queueId, entryId }: ExpandedViewProps) {
  ...
  const renderVisual = () => {
    ...
    if (type === 'dfrobot-light-rs485' && data instanceof dfrobot_light_rs485.RxEnvelope) {
      return <DfrobotLightRs485Expanded data={data} />;
    }
  ...
  const renderJson = () => {
    ...
    if (type === 'dfrobot-light-rs485' && data instanceof dfrobot_light_rs485.RxEnvelope) {
      const dfrobotData = dfrobot_light_rs485.RxEnvelope.toObject(data, {
        longs: String,
        enums: String,
        bytes: String,
        defaults: true
